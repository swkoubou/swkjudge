...
var textClock = {
    diff_date: 0,
    startTick: function (id, wait_time) {
        var self = this,
            $id = $("#" + id),
        ...
        self.syncServerDate(window.location.href);

        update_time();
    },
    syncServerDate: function (url) {
        var self = this;

        console.log("start synchronize to " + url);
        $.ajax({
            type: "GET",
            url: url,
            cache: false,
            success: function (data, status, xhr) {
                var server_date = new Date(xhr.getResponseHeader('Date'));
                if (server_date) {
                    self.diff_date = new Date() - server_date;
                    console.log("synchronize the time: " + server_date);
                }
            },
            error: function (xhr, status) {
                console.log("cannot synchronize: " + status);
            }
        });
    },

    getNow: function () {
        var date = new Date(new Date() - this.diff_date);
        return "" +
            date.getFullYear() + "/" +
     ...
     * @param x 驕ｩ逕ｨ縺吶ｋ蛟､
     * @param n 蟷
     * @returns {string} 0蝓九ａ縺輔ｌ縺溷､
     */
    zeroFill: function (x, n) {
        return ("00" + x).slice(-n);
    }
};
